...
function loadTable() {
    ...
    pessoaRequest.open("GET", "https://localhost:7038/api/pessoa");
  ...
  function showCidade() {
    ...
    const pessoaRequest = new XMLHttpRequest();
    pessoaRequest.open("GET", "https://localhost:7038/api/cidade");
    pessoaRequest.send();
    pessoaRequest.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            const objects = JSON.parse(this.responseText);
...
function pessoaEdit(id, nome, cpf, idade, cidadeid)
{
    ...
    xhttp.open("PUT", "https://localhost:7038/api/pessoa"+"/"+id);
...
function showPessoaCreateBox() {
  ...
  function pessoaCreate() {
    ...
    const xhttp = new XMLHttpRequest();
    xhttp.open("POST", "https://localhost:7038/api/pessoa");
    xhttp.setRequestHeader("Content-Type", "application/json;charset=UTF-8");
    xhttp.send(JSON.stringify({ 
  ...
  function cidadeCreate() {
    ...
    const xhttp = new XMLHttpRequest();
    xhttp.open("POST", "https://localhost:7038/api/cidade");
    xhttp.setRequestHeader("Content-Type", "application/json;charset=UTF-8");
    xhttp.send(JSON.stringify({ 
      "Nome": cname, "UF": cuf,
    }));
  ...
  function pessoaDelete(id) {
    const xhttp = new XMLHttpRequest();
    xhttp.open("DELETE", "https://localhost:7038/api/pessoa"+"/"+id);
    xhttp.send();
    xhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
          const objects = JSON.parse(this.responseText);
 ...
 function cidadeDelete(id) {
    const xhttp = new XMLHttpRequest();
    xhttp.open("DELETE", "https://localhost:7038/api/cidade"+"/"+id);
    xhttp.send();
    xhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
          const objects = JSON.parse(this.responseText);
